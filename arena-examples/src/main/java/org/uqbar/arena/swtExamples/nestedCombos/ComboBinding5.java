...
/**
 * Combo binding using ComboViewer
 * 
 * @author npasserini
 */
public class ComboBinding5 extends ApplicationWindow {
	...
	public ComboBinding5(Shell parentShell) throws Exception {
		super(parentShell);
	...
	private void bindControls() {
		...
		IObservableMap provincesAttributeMap = BeansObservables.observeMap(//
			BeansObservables.observeSet(Realm.getDefault(), this.model, "possibleProvinces"), Province.class, "name");

		cmbSerials.setLabelProvider(new ObservableMapLabelProvider(provincesAttributeMap));

		cmbSerials.setContentProvider(new ArrayContentProvider());

		bindingContext.bindValue(ViewersObservables.observeInput(cmbSerials),
			BeansObservables.observeValue(this.model, "possibleProvinces"), null, null);

		bindingContext.bindValue(ViewersObservables.observeSingleSelection(lstPartNumbers),
			BeansObservables.observeValue(this.model, "country"), null, null);
	}
}