...
/**
 * Binding example using POO
 *
 * @author npasserini
 */
public class ComboBinding3 extends ApplicationWindow {
	...
	public ComboBinding3(Shell parentShell) throws Exception {
		super(parentShell);
	...
	private void bindControls() {
		...
		IObservableMap attributeMap = BeansObservables.observeMap(
			Observables.staticObservableSet(countries), Country.class, "name");
		...
		bindingContext.bindList(SWTObservables.observeItems(cmbSerials.getCombo()),
			BeansObservables.observeDetailList(Realm.getDefault(), selectedPn, "provinces", Province.class),
