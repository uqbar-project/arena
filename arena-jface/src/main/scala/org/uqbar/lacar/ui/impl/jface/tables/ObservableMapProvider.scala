	...
	def getBackground(element:Object, columnIndex:Int):org.eclipse.swt.graphics.Color = {
		if(backgroundTransformers.contains(columnIndex)){
			...
			if(modelValue != null){
				...
				return getSWTColor(color)
			}
		}
		null
	}
	
	protected def getSWTColor(color:Color):org.eclipse.swt.graphics.Color  ={
		new org.eclipse.swt.graphics.Color(widget.getDisplay(), color.getRed, color.getGreen, color.getBlue);
	}
}